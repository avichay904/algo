...
class MarkdownRenderer extends Renderer {
  renderData() {
    ...
    const code = ({ value, language }) => (
      <pre dir="ltr"><code className={language ? `language-${language}` : undefined}>{value}</code></pre>
    );

    const inlineCode = ({ value, children }) => (
      <code dir="ltr">{value !== undefined ? value : children}</code>
    );
    ...
    return (
      <div className={styles.markdown} dir="rtl">
        <ReactMarkdown className={styles.content} source={markdown}
                       renderers={{ heading, link, image, code, inlineCode }}
                       escapeHtml={false}/>
      </div>
    );
  }
