...
class App extends BaseComponent {
  ...
  toggleHistoryBlock(enable = !this.unblock) {
    if (enable) {
      const warningMessage = t('app.discard_changes_confirm');
  ...
  loadAlgorithm({ categoryKey, algorithmKey, gistId }, { visualizationId }) {
    ...
    const fetch = () => {
      if (window.__PRELOADED_ALGORITHM__) {
        ...
        delete window.__PRELOADED_ALGORITHM__;
        return Promise.reject(new Error(t('app.algorithm_not_found')));
  ...
  render() {
    ...
    const title = `${saved ? '' : t('app.unsaved_prefix')}${titles.join(' - ')}`;
