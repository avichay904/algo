...
class Header extends BaseComponent {
  ...
  saveGist() {
    ...
    const save = gist => {
      if (!user) return Promise.reject(new Error(t('header.signin_required')));
      ...
      <header className={classes(styles.header, className)}>
        <div className={styles.row}>
          ...
          <div className={styles.section}>
            ...
            {
              !DISABLE_GIST && permitted &&
              <Button icon={faTrashAlt} primary onClick={() => this.deleteGist()} confirmNeeded>{t('header.delete')}</Button>
            }
            {
              !DISABLE_GIST && (
